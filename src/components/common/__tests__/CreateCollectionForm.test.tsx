...
describe('CreateCollectionForm', () => {
  ...
  describe('Rendering', () => {
    ...
    it('renders placeholder text in name input', () => {
      ...
      expect(screen.getByPlaceholderText(/e.g., work projects/i)).toBeInTheDocument()
    })

    it('renders back button when showBackButton is true', () => {
      const onBack = jest.fn()
      render(<CreateCollectionForm {...defaultProps} showBackButton onBack={onBack} />)
    ...
    it('does not render back button by default', () => {
      ...
      expect(screen.queryByRole('button', { name: /back to collections/i })).not.toBeInTheDocument()
    })
  })
  ...
  describe('Name Input', () => {
    ...
    it('calls onNameChange when typing', () => {
      ...
      render(<CreateCollectionForm {...defaultProps} onNameChange={onNameChange} />)
    ...
    it('has maxLength of 50 characters', () => {
      ...
      expect(screen.getByLabelText(/collection name/i)).toHaveAttribute('maxLength', '50')
    })
  })

  describe('Name Error', () => {
    it('displays name error message', () => {
      render(<CreateCollectionForm {...defaultProps} nameError="Name is required" />)

      expect(screen.getByText('Name is required')).toBeInTheDocument()
    })

    it('applies error styling to input when error exists', () => {
      render(<CreateCollectionForm {...defaultProps} nameError="Invalid name" />)

      expect(screen.getByLabelText(/collection name/i)).toHaveClass('border-red-300')
    })
  ...
  describe('Color Picker', () => {
    ...
    it('calls onColorChange when clicking a color', () => {
      ...
      render(<CreateCollectionForm {...defaultProps} onColorChange={onColorChange} />)

      fireEvent.click(screen.getByRole('button', { name: `Select color ${newColor}` }))
  ...
  describe('Back Button', () => {
    ...
    it('has chevron icon', () => {
      ...
      const backButton = screen.getByRole('button', { name: /back to collections/i })
